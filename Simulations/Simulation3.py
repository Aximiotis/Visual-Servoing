...
class DroneVision(Node):
    def __init__(self):
        ...
        self.e2 = 0
        self.counter=0
        self.Xn=np.zeros((3,3))
        self.Yn=np.zeros((3,1))
        self.A=np.array([0.1,0.1,0.7]).T
    ...
    def process(self):
        ...
        if label=="car" or label=="truck" or label=="bus":
            L_star= L.T @ np.linalg.inv((L @ L.T)+0.01*np.eye(3))
            ...
            saturated=np.array([np.clip(e[0]/50,-1,1),np.clip(e[1]/50,-1,1),np.clip(e[2]/50,-1,1)]).T
            V=-25*(L_star) @ e.T - 20*(L_star) @ np.array([self.e0,self.e1,self.e2]).T - 10*L_star@ saturated
            #25,20
            ...
            msg1.linear.x=float(V0linear[0])
            msg1.linear.y=float(V0linear[1])
            msg1.linear.z=0.0

            msg1.angular.x=0.0
            msg1.angular.y=0.0
            msg1.angular.z=float(0.4*np.clip(V0angular[2]/10,-1,1))

            self.Xn[:,0:2]=self.Xn[:,1:3]
            self.Xn[:,2]=np.array([V0linear[0],V0linear[1],msg1.angular.z]).T
         
            self.counter+=1

            if self.counter>=3:

                msg1.linear.x=float(self.Xn[0,:] @ self.A + 0.2*self.Yn[0])
                self.Yn[0]=msg1.linear.x

                msg1.linear.y=float(self.Xn[1,:] @ self.A + 0.2*self.Yn[1])
